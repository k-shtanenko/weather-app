...
func (s *ReportService) GenerateDailyReport(ctx context.Context, cityID int, date time.Time) (entities.ExcelReportEntity, error) {
	startOfDay := time.Date(date.Year(), date.Month(), date.Day(), 0, 0, 0, 0, date.Location())
	endOfDay := startOfDay.Add(24 * time.Hour).Add(-time.Second)
	return s.generateReport(ctx, entities.ReportTypeDaily, cityID, startOfDay, endOfDay)
}

func (s *ReportService) GenerateWeeklyReport(ctx context.Context, cityID int, year int, week int) (entities.ExcelReportEntity, error) {
	...
	return s.generateReport(ctx, entities.ReportTypeWeekly, cityID, periodStart, periodEnd)
}

func (s *ReportService) GenerateMonthlyReport(ctx context.Context, cityID int, year int, month time.Month) (entities.ExcelReportEntity, error) {
	periodStart := time.Date(year, month, 1, 0, 0, 0, 0, time.UTC)
	periodEnd := periodStart.AddDate(0, 1, -1)
	return s.generateReport(ctx, entities.ReportTypeMonthly, cityID, periodStart, periodEnd)
}

func (s *ReportService) generateReport(ctx context.Context, reportType entities.ReportType, cityID int, periodStart, periodEnd time.Time) (entities.ExcelReportEntity, error) {
...
func (s *ReportService) saveReport(ctx context.Context, reportType entities.ReportType, cityID int, periodStart, periodEnd time.Time, excelData []byte) (entities.ExcelReportEntity, error) {
	...
	ttl := 24 * time.Hour
	switch reportType {
	case entities.ReportTypeWeekly:
		ttl = 7 * 24 * time.Hour
	case entities.ReportTypeMonthly:
		ttl = 30 * 24 * time.Hour
	}
...
func (s *ReportService) GetReport(ctx context.Context, reportID string) (entities.ExcelReportEntity, error) {
