...
func TestWeatherService_GetStats(t *testing.T) {
	mockScheduler := &testutils.MockScheduler{}
	mockFetcher := &testutils.MockFetcher{}
	mockProducer := &testutils.MockProducer{}
	...
	assert.Equal(t, 3, stats["cities_count"])
	assert.Equal(t, "running", stats["status"])
	assert.Contains(t, stats, "timestamp")

	timestampStr, ok := stats["timestamp"].(string)
	assert.True(t, ok)
...
func TestWeatherService_FetchAndSendWeather(t *testing.T) {
	t.Run("successful fetch and send", func(t *testing.T) {
		...
		weathers := []entities.WeatherEntity{
			&entities.Weather{CityID: 123, CityName: "City1", Timestamp: time.Now(), Source: "test"},
			&entities.Weather{CityID: 456, CityName: "City2", Timestamp: time.Now(), Source: "test"},
		}
